...
- **AI 调用路径**：所有 LLM 请求均由**用户自己提供的 API Key** 直接从本地发起（通过 Rust 后端或前端直连），产品本身不架设中转服务器，因此用户的对话内容和屏幕截图永远不会经过 DeskCat 团队的任何服务器。
...
- **情感陪伴引擎**：驱动灵宠个性化形象的完整系统，包含图片状态机与 CSS 动画、自定义外观、桌面自由活动（跑酷、睡觉、互动）、随机对话触发、健康提醒等，负责“陪伴感”与“主动参与感”。
...
| 2D 动画 | **CSS 动画 + 帧切换**（基于透明 PNG）+ **Framer Motion** | Lottie、Rive、Spine | P0 因动画制作资源限制，采用设计师交付的 5 张关键姿态 PNG（正面、侧面、背面、睡觉、疑惑），通过 CSS `@keyframes` 实现呼吸、弹跳等微动效，利用 `framer-motion` 管理状态切换过渡。图像切换模拟行走（两帧交替），实现成本极低，且包体积零增加。后期 P2 替换为完整 Lottie 骨骼动画，当前方案作为占位和功能验证已足够。 |
...
**背景**：原设计选型 Lottie 骨骼动画，但因设计师资源排期，P0 无法交付完整的 `.lottie` 动画文件。团队交付了 5 张从原型照片分离的关键姿态 PNG。
**P0 方案**：使用这 5 张静态图，通过以下方式实现动态感：
- **呼吸/弹跳**：CSS `scale` / `translateY` 关键帧
- **行走**：`front` ↔ `side` 两帧快速交替 + 位移动画
- **状态表情**：`sleeping.png`、`wondering.png` 直接映射到 `sleeping` 和 `thinking` 状态
- **过渡**：`framer-motion` 的 `AnimatePresence` 让图片切换有淡入淡出效果
**优势**：
- 零动画制作成本，开发可立即启动
- 包体积极小（5 张 PNG 总计 < 200KB）
- 满足 P0 验证需求，所有状态均可视觉区分
**劣势**：
- 动画细腻度、连贯性不如 Lottie（如无骨骼变形，无长动作序列）
- 行走只有两帧，观感较机械
**长期计划**：P2 阶段设计师交付完整 `.lottie` 后，替换 `PetAvatar` 中的图片渲染逻辑为 Lottie 播放，其余状态机、活动引擎代码不变。此为“占位动画可替换”架构设计。
...
DeskCat 基于 **Tauri 2.0 的跨平台抽象层**，采用“一套代码，多平台编译”策略，无需为 macOS 和 Windows 分别维护独立代码库：
...
- **用户自定义灵宠形象**：P0 使用统一的默认png图片和css动画，不支持上传或更换形象。
- **云端同步**：所有数据纯本地。
- **自动更新**：版本更新通过官网下载手动完成。
...
**关键说明**：  
- 宠物窗不再只是“灵宠”，而是**整个产品的主界面**。其透明区域允许鼠标事件穿透，但灵宠本身和展开的对话框捕获鼠标。灵宠本体由 `<img>` 渲染当前状态对应的 PNG 图片，配合 CSS 动画和 `framer-motion` 实现动效。
...
- 与动画的衔接：`petStore.activityState` 与交互状态（`thinking` 等）通过优先级合并，决定当前显示的 PNG 图片和 CSS 动画类名。`PetAvatar` 组件根据最终状态选择图片源并附加对应动画。
...
2. Rust 命令：
   ...
   - 将截图保存为临时文件（如 `{temp_dir}/deskcat_screenshot_{timestamp}.png`），同时返回 base64 给前端。
3. 前端收到 base64 后：
...
P0 使用 5 张透明 PNG 图片，文件名与状态对应如下：

| 文件名 | 用途 | 适用状态 |
|--------|------|---------|
| `cat15-front.png` | 正面站立 | `idle`、`speaking`、`listening`、`happy`、`dragging` |
| `cat15-side.png` | 侧面行走帧 | `walking`、`running`（与 front 交替） |
| `cat15-back.png` | 背面（备选） | 暂未使用，预留给宠物移动到屏幕上方时展示背面 |
| `cat15-sleeping.png` | 趴睡 | `sleeping` |
| `cat15-wondering.png` | 歪头疑惑 | `thinking` |

**动画效果**（通过 CSS + Framer Motion 实现）：

| 状态 | 图片 | 附加动画 |
|------|------|---------|
| `idle` | `cat15-front.png` | 呼吸：`scale` 从 1 到 1.02 循环（4s ease-in-out） |
| `walking` | `cat15-front.png` ↔ `cat15-side.png` | 两帧交替（0.3s 切换），配合 `translate` 位移 |
| `running` | 同上 | 切换速度更快（0.15s），位移速度更高 |
| `sleeping` | `cat15-sleeping.png` | 极慢呼吸：`scale` 1.0 → 1.01（6s） |
| `thinking` | `cat15-wondering.png` | 轻微左右摇晃：`rotate` ±1deg（1.5s） |
| `speaking` | `cat15-front.png` | 添加跳跃式弹动：`translateY` -2px ~ 2px（0.3s） |
| `listening` | `cat15-front.png` | 静止，可加柔和的光晕脉冲 `box-shadow` |
| `happy` | `cat15-front.png` | 弹跳一次：`translateY` -10px（0.5s）后回原位 |
| `dragging` | `cat15-front.png` | 缩放至 1.05，加阴影，表示被抓取 |

**实现组件**：`PetAvatar.tsx`

- 使用 `framer-motion` 的 `<motion.img>` 切换图片，`animate` 属性动态绑定当前状态对应的动画变体。
- 状态切换时通过 `AnimatePresence` 添加淡入淡出过渡。
- 图片源和动画参数从 `animations.ts` 导出的映射表中读取。
...
**LLM 请求路径**：
- 所有 AI 请求由前端使用用户提供的 API Key 直接向 LLM 服务商发起（前端直连为主路径）。DeskCat 团队不架设任何中转代理服务器，因此用户的对话内容和屏幕截图绝不会经过 DeskCat 团队的服务器。
...
**子任务**：
- F1：将设计师交付的 5 张 PNG 放入 `public/assets/pet-images/` 目录，确认文件名正确。
- F2：创建 `features/pet/animations.ts`：
  - 定义状态到图片源和 CSS 动画配置的映射（如 4.8 节所述）。
...
**产出**：灵宠在桌面显示、可拖动、图片/动画随状态切换。  
**验证**：拖动灵宠到任意位置，右键菜单正常；调用 `petStore.setState('thinking')` 图片变为 wondering.png 且有摇晃动画。
...
- J4：与 CSS 动画/图片同步：活动状态变化 → 更新 `activityState` → PetAvatar 自动切换对应 PNG 图片和 CSS 动画
...
| 自由活动动画持续消耗 CPU | 影响笔记本续航 | 低 | CSS 动画在宠物窗不可见时暂停（`visibilitychange` 事件监听）；活动频率设为“低”时定时器延长至 120-300 秒；sleeping 状态时关闭 requestAnimationFrame 以纯 CSS 过渡替代行走的帧动画 |
