...
- 发现时间：2026-05-14
- 发现者：用户反馈
- 相关任务：Compact Window 与灵宠贴近定位
- 严重程度：一般
- 问题现象：大多数时候 compact window 和灵宠距离较近，但在某些位置，尤其灵宠靠近右下角时，compact window 会出现在屏幕偏中位置，和灵宠距离很远。
- 原因分析：`getCompactChatGeometry` 的候选位置包含屏幕居中项，并用 overflow/高度损失评分；在边界场景下，近邻候选可能因为高度或溢出惩罚失分，导致居中 fallback 被选中。算法没有把“贴近灵宠”作为第一排序目标。
- 解决方案：候选位置改为先生成灵宠下方、上方、左右四类近邻位置，并直接 clamp 到安全区；评分改为根据 compact 矩形与灵宠矩形的实际距离排序，再考虑高度损失和方向优先级。屏幕居中不再作为普通候选，只作为无近邻可用时的兜底。
- 涉及文件：`src/App.tsx`, `PROGRESS.md`, `ISSUES.md`
