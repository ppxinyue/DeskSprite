...
export function normalizeOrbSystemPrompt(prompt?: string | null): string {
  if (!prompt) return ORB_SYSTEM_PROMPT;
  try {
    const parsed = JSON.parse(prompt);
    return typeof parsed === 'string' && parsed.trim() ? parsed : ORB_SYSTEM_PROMPT;
  } catch {
    return prompt.trim() ? prompt : ORB_SYSTEM_PROMPT;
  }
}
...
export async function getActiveSystemPrompt(): Promise<string> {
  ...
  if (avatarRenderMode === 'orb' || avatarRenderMode === '"orb"') {
    return normalizeOrbSystemPrompt(await getSetting('orbSystemPrompt'));
  }
