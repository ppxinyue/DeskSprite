...
const EN: Record<string, string> = {
  ...
  '登录 macOS 后自动启动 DeskCat': 'Start DeskCat after logging in to macOS',
  ...
  '当用户正在进行以下游戏时，会自动取消置顶，并暂停 Timeline 刷新监测，以保证游戏性能。': 'When you are playing the games below, DeskCat automatically disables always-on-top and pauses Timeline monitoring to protect game performance.',
  '添加游戏': 'Add Game',
  '音乐识别': 'Music Detection',
  '只有这些音乐软件处于播放状态时，才会作为后台音乐标注到 Timeline。': 'Only these music apps are marked as background music in Timeline when they are actively playing.',
  '添加音乐软件': 'Add Music App',
  ...
  '添加关键词': 'Add Keyword',
  '屏蔽列表': 'Blocked List',
  '专注模式下，检测到这些软件或关键词会触发分心提醒。': 'In Focus Mode, these apps or keywords trigger distraction reminders.',
  '确认': 'Apply',
  ...
  '内置额度已用完，请配置个人 API。': 'Built-in quota is exhausted. Configure your own API.',
  '内置模型服务未配置，请配置个人 API。': 'Built-in model service is not configured. Configure your own API.',
  ...
  'STT 自定义配置': 'Custom STT Configuration',
  'TTS 自定义配置': 'Custom TTS Configuration',
  'Base URL、Model、API Key；标记为默认的配置会用于 Chat': 'Base URL, model, and API key. The default config is used for Chat.',
  'Base URL、Model、API Key；选择“使用此配置”后用于语音输入': 'Base URL, model, and API key. Choose “Use This Config” to use it for speech input.',
  'Base URL、Model、API Key；选择“使用此配置”后用于语音输出': 'Base URL, model, and API key. Choose “Use This Config” to use it for speech output.',
  ...
  '先写点内容，或添加一个文件。': 'Type something or add a file first.',
  '上传文件': 'Upload File',
  '暂不支持图片输入，请切换到支持视觉的模型。': 'Image input is not supported. Switch to a vision-capable model.',
  '未能从文档中提取文本，请换一个可复制文字的 PDF 或 DOCX。': 'Could not extract text from this document. Try a selectable-text PDF or DOCX.',
  '只能上传图片或文档，请选择 PNG、JPG、JPEG、WEBP、GIF、BMP、PDF 或 DOCX 格式。': 'Upload an image or document: PNG, JPG, JPEG, WEBP, GIF, BMP, PDF, or DOCX.',
  ...
  '内置额度已用完，请配置个人 API。 已切换到系统语音输入。': 'Built-in quota is exhausted. Configure your own API. Switched to system voice input.',
  '内置额度已用完，请配置个人 API。 已切换到系统朗读。': 'Built-in quota is exhausted. Configure your own API. Switched to system speech.',
  ...
  '请总结这个文档。': 'Please summarize this document.',
...
function translateDynamicEnglish(text: string): string {
  ...
  let next = text;
  next = next.replace(/^对话\s*(\d+)$/g, 'Conversation $1');
  next = next.replace(/^自定义：(.+)$/g, 'Custom: $1');
  next = next.replace(/^默认：(.+)$/g, 'Default: $1');
  next = next.replace(/^当前使用：(.+)$/g, 'Current: $1');
