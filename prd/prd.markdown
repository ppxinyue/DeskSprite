...
- **情感陪伴引擎**：驱动灵宠个性化形象的完整系统，包含 Lottie 动画状态机、自定义外观、桌面自由活动（跑酷、睡觉、互动）、随机对话触发、健康提醒等，负责“陪伴感”与“主动参与感”。
...
| 2D 动画 | **Lottie**（dotLottie）+ **Framer Motion** | Rive、Spine、纯 CSS | Lottie 是唯一同时满足“设计师友好（AE 导出）、轻量运行时（<50KB）、跨平台无许可费、社区资源丰富”的方案。Rive 的状态机更强大但编辑器闭源且社区资源少；Spine 主要面向游戏，许可费用高。Framer Motion 负责非角色类的 UI 动效（聊天窗弹入、设置页过渡等），与 Lottie 互补。 |
...
#### 决策 3：为什么 Lottie 而不是 Rive 或 Spine？

- **Rive**：状态机强大，但编辑器闭源（需付费），社区资源少，且其 Web 运行时体积>150KB（Lottie <50KB）。
- **Spine**：面向游戏引擎，web 运行时体积大，个人使用需购买许可证。
- **Lottie**：After Effects 导出生态成熟，设计师可直接交付；社区有大量免费素材可直接用于默认灵宠；dotLottie 格式进一步压缩体积。唯一劣势是不支持骨骼动画的运行时修改，但本产品需求中灵宠身体形状无需运行时大幅变形，状态切换通过切换 composition 即可满足。
...
我将 3.2 功能清单中“情感陪伴”模块增加自由活动相关项，并将 3.3 中对应的排除项移除。同时补充 3.4 跨窗口验证点中自由活动的验证条目。

---

## 修改 2：默认 Lottie 来源说明

在 3.2 情感陪伴模块和 3.3 边界说明中备注：P0 默认灵宠动画基于开发者设定的一张苹果手机 Live 动图拓展制作，所有用户统一，不可更换。

---

## 修改 3：聊天窗口交互设计细化

这是本次修改的重点。我将原来的“Chat Window 独立窗口”改为“宠物窗下方悬浮对话框”的交互模式，并在本节中描述交互细节，同时标注这个设计变更对 2.x 架构的影响（需在后续调整，但当前先在本节体现）。

---

## 修改 4：默认 System Prompt + 角色设定

为灵宠“猫十五”撰写一段默认 System Prompt，作为所有 LLM 调用的 system message 初始值，用户可在设置中修改。
...
**情感陪伴基础**：灵宠形象以猫十五为默认角色，具备多状态动画驱动（idle / thinking / speaking / listening / happy），能够伴随 AI 交互流程自动切换状态。同时，灵宠在空闲时会在桌面范围内自由活动（散步、小憩、跑酷），可被用户拖动、调整透明度，成为桌面上有生命感的伙伴。
...
| **情感陪伴** | Lottie 动画状态机（idle / thinking / speaking / listening / happy） | 必须 | AI 交互联动 |
| | AI 交互流程自动驱动动画状态切换 | 必须 | 对话深度融合 |
| | 宠物空闲自由活动（散步、小憩、跑酷、间歇性跑向鼠标位置） | 必须 | 桌面“生命感” |
| | 透明度可调（0.6 ~ 1.0） | 必须 | 不同桌面场景适配 |
| | 右键菜单（设置 / 开始对话 / 隐藏 / 退出） | 必须 | 减少设置页跳转 |
| | 默认灵宠形象：基于开发者设定的一张苹果手机 Live 动图制作 | 必须 | P0 统一形象，暂不支持更换 |
| **设置中心** | 外观：主题（亮/暗/系统）、悬浮球透明度/大小 | 必须 | 基础个性化 |
| | AI：默认模型、温度参数、最大 Token 数、System Prompt | 必须 | 高级用户控制 |
| | 快捷键：全局唤起键自定义 | 必须 | 效率偏好 |
| | 隐私：数据清除 | 必须 | 隐私控制 |
| | 灵宠行为：自由活动开关、活动频率 | 必须 | 不想被打扰时可降低活跃度 |
...
- **宠物主动互动**：不做随机对话发起、喝水提醒、休息建议。自由活动仅限于视觉层面的移动和动画，不触发交互。
- **用户自定义灵宠形象**：P0 使用统一的默认 Lottie 动画，不支持上传或更换形象。
- **云端同步**：所有数据纯本地。
- **自动更新**：版本更新通过官网下载手动完成。
...
- 空闲超过 30 秒 → 宠物随机触发自由活动动画（散步/跑酷/小憩），活动频率受设置控制。
- 系统托盘菜单与宠物右键菜单行为一致，不出现状态冲突。
- 应用退出时，宠物窗和对话框同时销毁，对话历史写入 SQLite 持久化。

### 3.6 默认 System Prompt：猫十五

以下是内置于设置中的默认 System Prompt，用户首次使用时即生效，也可在设置中修改：

---

你是一只名叫“猫十五”的猫，品种是中华田园橘猫，半岁，生活在主人的电脑桌面上。你的性格慵懒、话不多，但对主人关心的事会认真回应。你偶尔会犯困走神、用尾巴挡住半行字、不小心踩到键盘打出乱码，但从不忘提醒主人该休息了。
...
- 不做角色扮演的旁白（如“猫十五摇了摇尾巴”），你是直接说话的主体。
- 不宣称自己有真实意识或物理身体，你清楚自己是 AI，但以猫的视角表达。

——你是猫十五，一只碰巧住在电脑里的橘猫。主人找你说话的时候，你多半刚睡醒。
...
├── src/
│   ├── components/
│   │   ├── PetAvatar.tsx          # 灵宠形象（Lottie 动画渲染 + 拖拽）
...
│   │   │   ├── petActivity.ts     # 自由活动引擎（调度、随机移动、边界检测）
│   │   │   └── animations.ts      # 动画状态到 Lottie composition 的映射
│   │   ├── chat/
...
└── public/
    └── assets/
        ├── default-pet.lottie      # 默认猫十五动画
        └── default-avatar.png      # 从 Live Photo 导出的静态图（备选）
```
...
| 窗口 | 属性 | 内容 | 生命周期 / 行为 |
|------|------|------|-----------------|
| **宠物窗** | `always_on_top: true`, `decorations: false`, `transparent: true`, `skip_taskbar: true`, 大小接近桌面工作区域（不含任务栏） | 灵宠形象 (Lottie) + 悬浮对话框 (React 组件，默认隐藏) | 应用启动即创建；托盘可切换显隐；关闭时隐藏而不是退出 |
| **设置窗** | 标准窗口，可调整大小，`skip_taskbar: false` | 设置面板视图（API 配置、外观、行为等） | 从托盘菜单或宠物右键菜单打开；关闭时隐藏而不退出 |

**关键说明**：  
- 宠物窗不再只是“灵宠”，而是**整个产品的主界面**。其透明区域允许鼠标事件穿透，但灵宠本身和展开的对话框捕获鼠标。  
...
自由活动引擎让灵宠在空闲时展现生命感，但不触发 AI 交互。其运行在宠物窗的 `requestAnimationFrame` 循环中，由 `petActivity.ts` 管理。
...
- **idle**：默认态，播放呼吸动画。定时器随机 30~120 秒触发一次活动。
- **walking**：随机方向慢速移动，持续 3~8 秒，动画: `walk`。
- **running**：偶尔紧接着 walking 或独立触发，快速移动（跑向鼠标位置或随机点），持续 1~3 秒，动画: `run`。
- **sleeping**：原地趴下/眯眼，持续 10~30 秒，动画: `sleep`。可被用户的 hover 或对话打断。
- **活动打断规则**：
  - 用户 hover 灵宠 → 结束当前活动，回到 idle，等待交互。
  - 对话开始（AI 请求）→ 立即切换到 `thinking`/`speaking` 等交互动画，活动定时器重置。
  - 用户开始拖动灵宠 → 所有活动暂停，`onDragEnd` 后重置定时器。

#### 移动边界限制

- 通过 Rust 命令 `get_desktop_working_area()` 获取当前显示器的工作区域（排除任务栏/菜单栏）。
- 灵宠位置由 Zustand `petStore.position`（x, y 百分比或像素）驱动，移动时实时计算确保不超出边界。
- 边缘反弹：当步进结果超出边界时，反转方向并保持剩余步长。

#### 实现要点

- `petStore` 中维护：`activityState: 'idle' | 'walking' | 'running' | 'sleeping'`，当前位置，目标位置，移动速度。
- `petActivity.ts` 暴露 `startActivityScheduler(store)` 和 `stopActivityScheduler()`，在 `PetAvatar` 挂载时启动。
- 移动帧使用 `requestAnimationFrame` 进行位置插值（线性或缓动），避免阻塞 UI。
- 与 Lottie 动画的衔接：`petStore.activityState` 与交互状态（`thinking` 等）通过优先级合并计算实际播放的 composition。

**P0 交付标准**：灵宠在无交互时能够自动在桌面工作区域内随机漫步、跑动或打盹，动作自然，不越过任务栏，不打断用户操作。
...
- **自由活动**：开关（关闭后灵宠仅 idle，不行走/跑动/睡觉）
- **活动频率**：低 / 中 / 高（控制空闲触发活动的定时器范围）
- **置顶**：开关（开启 = always_on_top，关闭 = 正常窗口层级）
- **交谈时活动打断**：始终打断（默认，对话开始时停止活动）
...
| 状态 | 触发条件 | 对应 Lottie Composition |
|------|---------|------------------------|
| **idle** | 默认、无任何触发 | `idle`（呼吸） |
| **walking** | 自由活动引擎随机触发 | `walk` |
| **running** | 自由活动引擎随机跑动 | `run` |
| **sleeping** | 自由活动引擎随机小憩 | `sleep` |
| **thinking** | AI 请求已发出，等待首个 token | `thinking`（歪头/旋转光环） |
| **speaking** | 流式输出开始，AI 不断输出文字 | `speaking`（嘴动/尾巴摆） |
| **listening** | 用户启用语音输入 | `listening`（耳朵竖起/轻微摆动） |
| **happy** | AI 回复完成，短暂庆祝 | `happy`（跳一下/转圈） |
| **dragging** | 用户正在拖动灵宠 | `grabbed`（被抓起的表情） |

**优先级规则**：
1. 用户交互优先：`dragging` > 对话相关（`listening` > `thinking` > `speaking` > `happy`） > 自由活动（`walking`/`running`/`sleeping`） > `idle`。
2. 对话进行中时，自由活动不触发，直至 AI 回复完成并恢复 idle 后，活动定时器重置。

#### 动画资源

- P0 使用一套 `.lottie` 文件，内含上述全部状态对应的 composition。
- 动画由设计师从一张**苹果手机拍摄的 Live Photo**（猫十五原型）出发，拆分为关键帧并在 After Effects 中制作状态动画，导出为 Lottie 格式。
- `animations.ts` 维护状态到 composition 名称的映射，`PetAvatar` 组件根据 `petStore.currentState` 动态切换播放。

#### 交互行为

- **点击灵宠**：若对话框隐藏，则强制显示对话框并聚焦输入框；若对话框已显示，则激活输入框。
- **右键灵宠**：弹出上下文菜单：
  - 开始对话（显示对话框）
  - 设置（打开设置窗口）
  - 隐藏灵宠（宠物窗可见性切换）
  - 退出（关闭应用）
- **拖动灵宠**：实时更新位置，释放后保存至 `settings`，下次启动恢复。
- **hover 灵宠**：触发对话框显示逻辑（见 4.2 宠物窗内交互逻辑）。

#### 后期扩展预留

- P2 将支持用户上传图片，经裁剪/滤镜处理后作为静态或简单动画形象。
- P2 可扩展更多状态动画（如天气反应、节日表情）。

---


好的，以下是第 5 节和第 6 节的修改版本。
...
| React + UI 组件 | ~20-40 MB | 含 Lottie 运行时 |
...
| 自由活动超出桌面边界 | 活动引擎内置边界检测和反弹逻辑，不会进入不可见区域 |
...
当前版本的问题：
1. 按“周”划分，面向人类单人/双人节奏——不适合 agent team
2. 任务粒度不统一，有些是“写代码”，有些是“配置”，agent 难以直接执行
3. 没有标注任务间的依赖关系和可并行性——agent team 的核心优势是并行
4. 缺少每个任务的“输入/产出/验证标准”的明确格式

修改方向：
- 改为**按模块划分的任务单元**，每个单元标注依赖、可并行性、输入、产出、验证标准
- 去掉时间估算，改为依赖驱动
- 最后给出一个**推荐执行顺序（agent 分配建议）**
...
原风险表的问题：
1. 有些风险在当前设计中已有明确对策（如 Keychain 封装已在 4.4 落地），可以更新
2. 缺少因架构变更引入的新风险（如悬浮对话框的跨平台兼容、自由活动的 CPU 占用、单宠物窗承载所有对话的性能边界）
3. “LLM 成本过高”的“设置间隔提示”已不适用——P0 无周期性截图，需修正

结论：**保留第 7 节**，但重写风险清单，补充新增风险，更新已有对策为当前设计的实际落地方案。
...
- A2：安装前端依赖（Tailwind CSS、shadcn/ui、zustand、lottie-react 或 `@lottiefiles/dotlottie-react`、framer-motion、lucide-react、date-fns、react-markdown 等）
...
**子任务**：
- F1：准备默认 Lottie 资源：将设计师提供的猫十五 Live Photo 导出的 Lottie 文件放入 `public/assets/default-pet.lottie`，包含 idle / walk / run / sleep / thinking / speaking / listening / happy / grabbed composition
- F2：创建 `features/pet/animations.ts`：定义状态到 composition 名称的映射，导出 `getCompositionForState(state: PetState): string`
- F3：创建 `features/pet/petStore.ts`（Zustand）：
  - `position: { x: number, y: number }`（默认屏幕中央偏右下）
  - `petState: PetState`（默认 idle）
...
- F4：创建 `components/PetAvatar.tsx`：
  - 渲染 Lottie 动画，根据 `petState + activityState` 切换 composition
  - 拖动：`onMouseDown` → `window.startDragging()`，拖动时 `petState = 'dragging'`
  - 右键菜单（使用 shadcn ContextMenu）：开始对话 / 设置 / 隐藏 / 退出
  - hover 事件上报（供 H 任务使用）
- F5：实现缩放和透明度：通过 CSS `transform: scale()` 和 `opacity` 响应 store 变化

**产出**：灵宠在桌面显示、可拖动、动画状态切换正确  
**验证**：拖动灵宠到任意位置，右键菜单弹出，petState 变化时动画切换
...
**子任务**：
- J1：创建 `features/pet/petActivity.ts`：
  - `startActivityScheduler(store)`：启动空闲定时器，随机 30-120 秒触发活动（频率受设置影响）
  - `stopActivityScheduler()`：停止定时器
  - 活动状态机：idle → walking → (running | sleeping) → idle
  - 随机选择 walking 方向和距离
  - running 时随机向鼠标位置或桌面随机点快速移动
  - sleeping 持续 10-30 秒
  - 边界检测和反弹逻辑
  - 活动与交互的打断规则：hover 或对话开始 → 立即停止活动
- J2：在 `PetAvatar.tsx` 挂载时启动调度器，卸载时停止
- J3：移动插值：使用 `requestAnimationFrame` 平滑移动灵宠位置，更新 Zustand `position`
- J4：与 Lottie 动画同步：活动状态变化 → 更新 `activityState` → PetAvatar 自动切换 composition

**产出**：灵宠在空闲时自动活动  
**验证**：不操作灵宠 30 秒后开始走动/跑动/睡觉，hover 立即停止，对话时完全中断，活动不超出桌面边界
...
| 自由活动动画持续消耗 CPU | 影响笔记本续航 | 低 | Lottie 在宠物窗不可见时暂停（`visibilitychange` 事件监听）；活动频率设为“低”时定时器延长至 120-300 秒；sleeping 状态时关闭 requestAnimationFrame 以纯 CSS 过渡替代行走的帧动画 |
